...
public class checkresultcopy : MonoBehaviour
{
    ...
    void Start()
    {
		...
		Chord = new int[6]{0,0,0,0,0,0};
		PredictTrueChord = new int[6]{0,0,0,0,0,0};
		PredictFalseChord = new int[6]{0,0,0,0,0,0};
        List<string> ChordList = new List<string>(){"A","Am","C","D","E","Em","F","G"};
        List<string> RealChord = new List<string>(){"A","C","D","E","Em","F","G"};
		// for(int i = 0; i < 6; i++)
		// {
		// 	Chord[i] = 0;
        ...
        for (int i = 0  ; i < RealChord.Count ; i++)
        {
            ...
            int [] pre_confused = new int[6]{0,0,0,0,0,0};
            foreach (var chord in listStrLineElements)
            {
                if(chord == RealChord[i])
                {
                    if(RealChord.IndexOf(StaticClass.CrossSceneTrueInformation[CountCh]) == -1)
                    {
                        ...
                        else if(ChordList.IndexOf(StaticClass.CrossSceneTrueInformation[CountCh]) == 5)
                        {
                            pre_confused[3] = pre_confused[3] + 1;
                        }
                        else
                        {
                            continue;
                    ...
                    {
                        if(RealChord.IndexOf(StaticClass.CrossSceneTrueInformation[CountCh]) > 3)
                        {
            ...
            if(i == 4)
            {
                for(int j = 0; j < 6; j++)
                {
                    confused_mat[3][j] += pre_confused[j];
                }
            }
            else if(i > 3)
            {
